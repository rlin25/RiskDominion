...
- [Technical Terms](#technical-terms): Claude, Fire-and-Forget, LLM, Procedure, Reducer, Replay Mode, Scheduled Procedure, Scheduled Reducer, SpacetimeDB, Spectator Mode, Subscription, View
...
**Orchestration**
The architecture in which multiple AI agents work together on a single task. In Slice 5, each AI opponent's reasoning cycle uses orchestration: from within the reasoning procedure, four specialist Claude calls are issued over `ctx.http`, each analyzing one domain of the game state with a narrow focused prompt. Once all four return, the commander calls Claude again with all four specialists' recommendations and produces the final decision. This contrasts with the single-agent approach in Slices 2 through 4, where one Claude call handled the full reasoning. See also: Commander, Specialist, Deliberation Chain, Procedure.
...
**Reasoning Cycle**
The roughly 60-second interval at which each AI opponent analyzes the current game state and submits actions. It runs as a scheduled procedure (the only function type that can call Claude over HTTP). In Slices 2 through 4, each cycle makes one Claude call. In Slice 5, each cycle makes five Claude calls from within the one procedure (four specialists plus one commander). The cycles are staggered: Zhao, Consortium, and Prophet start offset from one another, and each cycle re-arms its own next run. If a cycle is still in flight when its next run fires, that run is skipped. A 30-second request timeout (Slice 5: commander) prevents the game from freezing on slow API responses. See also: AI Opponent, Orchestration, Deliberation Chain, Scheduled Procedure.

**Specialist**
In Slice 5, one of four domain-focused Claude agents that analyze a subset of the game state for an AI opponent. Each AI has four specialists: one for Military, one for Economic, one for Cultural, and one for Covert. Each specialist receives only the game data relevant to its domain (not the full state), uses a focused prompt with the AI's persona context, and returns up to 3 recommended actions with one-sentence justifications. All four specialist calls are issued from the same reasoning procedure over `ctx.http`. Their outputs are passed to the Commander. See also: Commander, Orchestration, Deliberation Chain.
...
**LLM**
Large Language Model. A type of AI system trained to understand and generate natural language. In Risk: Dominion, the LLM used is Claude. LLM calls are made server-side from procedures (the only function type that may make HTTP calls), each with a request timeout. All prompts include a structured game state snapshot and explicit output format instructions to ensure consistent, parseable responses. See also: Claude, Procedure.

**Procedure**
A server-side function in SpacetimeDB that can return a typed value to the caller and is the only function type allowed to make outbound HTTP calls, via `ctx.http`. Procedures take `&mut ProcedureContext` and are not auto-transactional: database access goes through `ctx.with_tx(|tx| { ... })`. Every Claude call in Risk: Dominion happens in a procedure -- the AI reasoning cycle, the Strategist cycle, natural language queries, canned queries, and Tab autocomplete. The intel query is also a procedure (it returns data) even though it makes no HTTP call. The standard pattern is snapshot in one transaction, make the HTTP call with no transaction held open, then commit results in a second transaction. Clients call procedures and receive the return value directly. See also: Reducer, Scheduled Procedure, Claude, SpacetimeDB.

**Reducer**
A deterministic, sandboxed server-side function in SpacetimeDB that validates and mutates database tables in a single transaction. When you drag a card onto a territory, the client calls a reducer. The reducer checks whether the action is legal (correct territory, sufficient action points, valid adjacency) and mutates the relevant tables if so. A reducer returns `Result<(), String>`: success commits its writes, and an error rolls them back and surfaces a message. A reducer CANNOT make HTTP calls and CANNOT return data to the client -- the client observes the outcome through its table subscriptions, not through a return value. Reducers are the only way to change game state directly; no client can write to a table. See also: Procedure, SpacetimeDB, Subscription.
...
**Scheduled Procedure**
A procedure that fires automatically on a timer, driven by a scheduled table, rather than by a client call. Because it is a procedure, it can make HTTP calls. Risk: Dominion uses scheduled procedures for the Claude-driven cycles: the AI reasoning cycle (one in-flight row per AI, ~60 seconds, staggered starts) and the Strategist advisor cycle (~60 seconds, offset ~50 seconds). Each AI cycle re-arms its own next run with a one-shot `ScheduleAt::Time`, so the cycles self-pace. See also: Procedure, Scheduled Reducer, Reasoning Cycle.

**Scheduled Reducer**
A reducer that fires automatically on a fixed time interval, driven by a scheduled table, without being triggered by a player. Because it is a reducer, it makes no HTTP calls -- it runs pure database logic. Risk: Dominion has two scheduled reducers: action point regeneration (every 8 seconds) and the cultural spread tick (every 30 seconds). The Claude-driven cycles (AI reasoning, Strategist) are scheduled procedures, not scheduled reducers. Scheduled functions are how time-based game mechanics are implemented inside the database. See also: Reducer, Scheduled Procedure, SpacetimeDB.

**SpacetimeDB**
The database platform (version 2.4.1) that powers Risk: Dominion's backend. SpacetimeDB runs the database and the server in the same process -- there is no separate web server or API layer. Tables, reducers, procedures, views, subscriptions, and scheduled functions all live inside SpacetimeDB. Clients connect directly via WebSocket (using the `spacetimedb` npm package) and receive real-time table updates through subscriptions. See also: Reducer, Procedure, Subscription.
...
**View**
A read-only query function in SpacetimeDB, the third server function type alongside reducers and procedures. Views expose server-side reads to clients without mutating state. Risk: Dominion leans on table subscriptions for live state and uses views sparingly. See also: Reducer, Procedure, Subscription.
